...
import argparse
import os
from data_utils.PRICONV2DataLoader import PRICONV2DatasetWholeScene
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='2024-08-27_01-36', help='experiment root')
...
def main(args):
    ...
    visual_recon_dir = experiment_dir + '/visual_1/'
    visual_recon_dir = Path(visual_recon_dir)
    ...
    root = '../data/s3dis/priconv2_3d/'

    TEST_DATASET_WHOLE_SCENE = PRICONV2DatasetWholeScene(root, split='test', test_group=args.test_group, block_points=2048)
    ...
    if not os.path.exists(checkpoint_path):
        log_string(f"Checkpoint file not found: {checkpoint_path}")
    ...
    try:
        ...
        log_string(f"Failed to load checkpoint: {e}")
        raise e
        ...
        for batch_idx in range(num_batches):
            ...
            for _ in tqdm(range(args.num_votes), total=args.num_votes):
                ...
                for sbatch in range(s_batch_num):
                    ...
                    seg_pred, recon_pred, _ = classifier(torch_data)
                    ...
                    recon_pred_np = recon_pred.contiguous().cpu().data.numpy()  # Convert to numpy
                    all_scene_reconstructions.extend(recon_pred_np)  # Store reconstructions for this scene
            ...
            if args.visual:
              ...
              # Save the predicted (x, y, z) values for this scene in the visual_1 folder
            all_scene_reconstructions_np = np.array(all_scene_reconstructions)
            np.save(os.path.join(visual_recon_dir, f'{scene_id[batch_idx]}_predicted_reconstructions.npy'), all_scene_reconstructions_np)
